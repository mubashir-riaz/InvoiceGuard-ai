...
import { Menu, Bell, Database } from "lucide-react";

const Layout = () => {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const location = useLocation();

  // Close sidebar overlay on ESC key press
  useEffect(() => {
  ...
  // Close sidebar overlay when location changes
  useEffect(() => {
    setSidebarOpen(false);
  }, [location.pathname]);
  ...
  return (
    <div className="min-h-screen bg-slate-50/50">
      {/* Overlay Navigation Sidebar - Slides over content without moving page layout */}
      <Navbar isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} />

      {/* Main content area - Stationary and full width (no content shift) */}
      <div className="min-h-screen flex flex-col">
        {/* Header Bar */}
        <header className="sticky top-0 z-30 flex items-center justify-between h-16 px-4 sm:px-6 bg-white/80 backdrop-blur-md border-b border-slate-100">
          {/* Left section: Drawer Toggle & Title */}
          <div className="flex items-center gap-3 sm:gap-4">
            <button
              onClick={() => setSidebarOpen((prev) => !prev)}
              className="p-2 rounded-xl text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
              title="Open menu"
